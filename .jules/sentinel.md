...
## 2024-05-24 - Prototype Pollution via Unvalidated External Keys

**Vulnerability:** The `byTrip` dictionary in the GTFS-RT worker used `{}` for initialization, which exposed it to prototype pollution if a malicious feed ID (like `__proto__`) was processed.
**Learning:** Plain JavaScript objects inherit from `Object.prototype`, meaning special keys like `__proto__`, `constructor`, or `hasOwnProperty` can overwrite inherited properties if unvalidated external string keys are used.
**Prevention:** To prevent prototype pollution vulnerabilities when using plain JavaScript objects as dictionaries for unvalidated external string keys (like GTFS-RT feed IDs), always initialize them with `Object.create(null)` instead of `{}`.
