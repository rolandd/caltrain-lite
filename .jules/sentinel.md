...
## 2026-07-31 - Prototype Pollution via Object Keys

**Vulnerability:** A plain JavaScript object (`{}`) was used as a dictionary for external string keys (`tripId` from GTFS-RT).
**Learning:** Using `{}` for dictionaries with external, unvalidated string keys opens the door to prototype pollution. If a key like `__proto__` is parsed, it could mutate the `Object.prototype`, affecting all objects in the runtime environment.
**Prevention:** Always use `Object.create(null)` for dictionaries when keys are sourced from external data or unvalidated input to create objects with no prototype.
