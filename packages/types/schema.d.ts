...
// ---------------------------------------------------------------------------
// Performance & Historical Delay Estimation
// ---------------------------------------------------------------------------

/**
 * Historical on-time performance profile generated by daily GHA processing.
 * Stored in Cloudflare KV (`performance:data`).
 * Fetched by PWA via `GET /api/performance` and cached in IndexedDB.
 */
export interface TrainPerformanceProfile {
  /** Map of Train Number (e.g. "101") to trip performance metrics. */
  trips: Record<string, TripPerformance>;
  /** Corridor track leg distances in meters keyed by `"originStopId->destStopId"`. */
  legDistances?: Record<string, number>;
  /** Window metadata. */
  meta: {
    generatedAt: number; // Epoch seconds
    windowDays: number; // Rolling window size (e.g., 90)
    sampleSize: number; // Total completed trip runs analyzed
  };
}

export interface TripPerformance {
  /** Stop-level baseline metrics indexed by canonical station ID. */
  stops: Record<string, StopPerformance>;
  /** Sub-leg expected speed profiles indexed by `"originStopId->destStopId"`. */
  legs?: Record<string, LegPerformance>;
}

export interface StopPerformance {
  /** Median historical delay in seconds at arrival (p50). */
  p50Delay: number;
  /** 90th percentile delay in seconds at arrival (p90). */
  p90Delay: number;
  /** Median dwell duration in seconds at this station. */
  dwellSec: number;
}

export interface LegPerformance {
  /** Observed median travel duration in seconds for this leg. */
  medianTravelSec: number;
  /** P90 travel duration in seconds for this leg. */
  p90TravelSec: number;
