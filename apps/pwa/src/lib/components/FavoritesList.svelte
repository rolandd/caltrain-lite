    ...
    <div class="grid grid-cols-1 gap-3">
      {#each favorites as pair (pair)}
        ...
        >
          ...
          <button
            class="text-transit-favorite text-xl cursor-pointer bg-transparent border-none p-2 rounded-full hover:bg-transit-surface-hover-soft transition-colors active:scale-95 flex items-center justify-center leading-none"
            onclick={(e) => {
              e.stopPropagation();
